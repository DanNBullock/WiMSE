...
In a previous lesson with digital images, we explored the notion of a map/parcellation.  In this chapter we'll do the same for NIfTI data, and see how parcellations are implemented in this context.  Let's begin by loading a parcellation generated by [FreeSurfer](https://surfer.nmr.mgh.harvard.edu/) a software package for neuroimaging data which can, among other things, automatedly segment structural brain data (i.e. a T1) into [known structures and/or areas](https://surfer.nmr.mgh.harvard.edu/fswiki/CorticalParcellation).  Note that this parcellation *is specific to* a particular subject and data source, in this case, it was derived from the subject and source (T1) that we viewed in the previous lesson.
...
It seems that our frequency values are traversing several orders of magnitude.  Just for the sake of easy visualization, let's leave off the top three most common values (0, 41, 2) and plot 3 histogram of the rest, splitting them at values below 10000, between 10000 and 12000, and above 12000.

Some values we can take note of right off the bat are 0, 2 and 41.  These correspond to the background (i.e. unlabeled entries), left cerebral white matter, and right cerebral white matter.  The extremely large value associated with "background" (15382362) [] is indicative of what proportion of the data volume is actually associated with labeled entries. Likewise, the values for left and right cerebral white matter (268665 and 265864) are indicative of the large amount of brain "real estate" occupied by the undifferentiated white matter labels.  Ultimately, it will be our task as budding white matter segmenters to divide those areas up more sensibly, but for now let's look at a plot of the relative size of the brain volumes occupied by these labels.  We have opted not to plot the data for background, left and right cerebral white matter because of how they would informatively dwarf the other values.  Also, keep in mind, that given the resolution of the atlas we are using (1 mm^3, isotropic) the values you note in the subsequent plot can be interpreted as cubic millimeters _or_ as milliliters (so as to compare to intuitive reference volumes like a standard 12oz soda can, which contains ~ 355 ml of liquid).
...
Let's limit our view of the table to consider only those labels that are present in our current nifti object *and* augment the table column or how many pixels correspond to that entry.

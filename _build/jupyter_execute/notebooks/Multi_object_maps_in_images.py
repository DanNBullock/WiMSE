...
As we did in the previous lesson, let's go ahead and compute the color distances from this water color.  We'll then plot these distances as we did previously, so we can get a sense of the range of color distances in the map.
...
Lets try and come up with a good threshold color distance we can use to make a mask for this data.  Keep in mind that this is only a rough approximation, and that we aren't going to get a perfect mask with this method no matter how hard we try here.
...
Now that we have both masks, let's try and shift the political map onto the satellite map.
...
With a bit of experimentation, it should be possible to get the disagreement percentage down to around 10.5% , with an X offset of between 19 and 30 and a Y offset of -8.
...
colorArray[0:100,0:99,:]=pinkPixel.astype(int)
colorArray[0:100,100:199,:]=orangePixel.astype(int)
colorArray[0:100,200:299,:]=redPixel.astype(int)
colorArray[0:100,300:399,:]=greenPixel.astype(int)
colorArray[0:100,400:500,:]=yellowPixel.astype(int)
...
Now that we have this "paint by numbers" representation, let's look at unique "masks" that we can generate using this data.  In this case, yellow will indicate pixels matching the current color of interest, while blue will indicate pixels that *do not* match the current color of interest.
...
In the next section we will begin bridging into neuroimaging data.  We'll start this process by briefly setting up some intuitions about the similarities and differences between 2-D digital images and brain images, before moving on to a more explicit exploration of the particular data object features of brain images. 
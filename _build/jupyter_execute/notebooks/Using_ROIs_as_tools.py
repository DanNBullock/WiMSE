...
- 1.  The "planeCoord" for the initial plane (as before)
- 2.  The dimension for the initial plane (as before)
- 3.  The "planeCoord" for the new "cut" plane
- 4.  The dimension for the new "cut" plane
- 5.  Which portion of the initial plane to keep after the cut has been performed
...
Lets try performing another cursory segmentation, but this time we'll use a sphere.  Because of how a sphere works in practice (at least as a singleton criteria) we'll constrain the demo to using the ROI as an **inclusion** criterion.  As such we'll perform the segmentation in two steps, only one of which is interactive:

- 1.  Define the location and radius of the sphere
- 2.  Apply the segmentation criteria and view the output

Hint: a good demonstration of how to use a sphere in a segmentation requires a careful application.  Try placing your sphere at the [splenium of the corpus callosum](https://en.wikipedia.org/wiki/Corpus_callosum#Structure) using the coordinate [0,25,0].
...
The output of a sphere-based segmentation (and its sensibility/interpretability) is _highly_ dependent on the placement of the sphere's centroid and the selection of its radius.  If you used the recommended coordinate for the [splenium of the corpus callosum](https://en.wikipedia.org/wiki/Corpus_callosum#Structure), [0,25,0], you ought to have obtained a streamline collection that roughly corresponds to the forceps major.  Alternatively, if you selected a different location (or perhaps selected a radius that was too large) it is possible that your result may resemble more of a hairball than a coherent structure.  This is because a spherical ROI, when used as an inclusion criterion (and when used to detect _any_ volumetric traversal), can be just as generous as a planar ROI, if not moreso.

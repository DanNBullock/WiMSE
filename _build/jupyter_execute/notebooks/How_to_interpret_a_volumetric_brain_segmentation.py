...
In a previous lesson with digital images, we explored the notion of a map/parcellation.  In this chapter we'll do the same for NIfTI data, and see how parcellations are implemented in this context.  Let's begin by loading a parcellation generated by [FreeSurfer](https://surfer.nmr.mgh.harvard.edu/) ([Fischl 2012](https://dx.doi.org/10.1016%2Fj.neuroimage.2012.01.021)) a software package for neuroimaging data which can, among other things, automatedly segment structural brain data (i.e. a T1) into [known structures and/or areas](https://surfer.nmr.mgh.harvard.edu/fswiki/CorticalParcellation).  Note that this parcellation *is specific to* a particular subject and data source.  In this case, it was derived from the subject and source (T1) that we viewed in the previous lesson.
...
Let's take a quick moment to look at how this compares to our source T1.
...
Let's compare a few of these fields.  Specifically:  
...
#display volume occupied for T1
...
We'll come back to this as we try and align these images.  For now, let's move towards a consideration of the actual data content of the parcellation data object. 
...
Just as we did before with our T1 data, let's also consider the frequency of these values as well.
...
AtlasLabelCounts.head(25)

It seems that our frequency values are traversing several orders of magnitude.  Just for the sake of easy visualization, let's leave off the top three most common values (0, 41, 2) and plot three histograms of the rest, splitting them at values below 10000, between 10000 and 12000, and above 12000.

Let's consider these three common values for a moment, though.  These values correspond to the background (i.e. unlabeled entries), left cerebral white matter, and right cerebral white matter.  The extremely large value associated with "background" (15382362) is indicative of the small proportion of the data volume that is actually associated with labeled entries. Likewise, the number of voxels for left and right cerebral white matter (268665 and 265864) are indicative of the large amount of brain "real estate" occupied by the undifferentiated white matter labels.  Ultimately, it will be our task as white matter segmenters to divide those areas up more sensibly, but for now let's look at a plot of the relative size of the brain volumes occupied by these labels.  We have opted not to plot the data for background, left and right cerebral white matter because of how they would dwarf the other values.  Also, keep in mind, that given the resolution of the atlas we are using (1 mm^3, isotropic) the values you note in the subsequent plot can be interpreted as cubic millimeters _or_ as milliliters (so as to compare to intuitive reference volumes like a standard 12oz soda can, which contains ~ 355 ml of liquid).
...
Here we see the list of possible label numbers (under **#No**), the the names of the anatomical regions they correspond to (under **LabelName**), and the internal color table assigned by Freesurfer (for plotting applications in FreeSurfer) under **R,G, & B**.  Although there are over 1,000 entries here, any given parcellation will only have a subset of these.  This is because [this table](https://surfer.nmr.mgh.harvard.edu/fswiki/FsTutorial/AnatomicalROI/FreeSurferColorLUT) (taken from the FreeSurfer website) actually covers several, mutually exclusive, parcellation schemas.  In this way, two labels/numbers may correspond to overlapping areas of cortex (and thus the same constituent voxels of the associated T1 image), but _not_ in the same data structure.  For example, this would be the case with the label for the superior frontal gyrus from the Desikan-Killiany atlas[citation]--1028-- the superior frontal gyrus from the ["Destrieux" 2009 atlas](https://dx.doi.org/10.1016%2Fj.neuroimage.2010.06.010)--11116.  To reiterate though, in any given parcellation/atlas data object, any particular voxel could only be associated with *one* of these numbers/labels, and this would be determined by the parcellation schema (i.e. ["ontology"](https://en.wikipedia.org/wiki/Ontology_(information_science))) that the parcellation data object was derived from. 

Let's limit our view of the table to consider only those labels that are present in our current NIfTI object *and* augment the table column with how many pixels correspond to that entry.
...
- The "Unknown" entries are extremely numerous.  These constitute the background voxels--those for which no explicit anatomical mapping exists.  Thus, even voxels without a direct mapping still have a numerical label attached to them.
- Numbers below 1000 (at least of the entries we see) correspond to subcortical structures
...
Now that we have taken a look at our atlas's labels and their voxel counts, lets see how they are spatially laid out in the data volume.
...
As you use the above sliders to move through the parcellation there are several things to take note of.  First and perhaps most striking,  while the sagittal view may be appropriately labeled, it is oriented in a somewhat disorienting fashion (typically we might expect this to be oriented such that the left and right axes of the plot correspond to the anterior-posterior axis of the brain).  The [coronal and axial](https://en.wikipedia.org/wiki/Anatomical_plane) slices are switched, but even switching them back wouldn't be a complete solution, as the mislabeled axial slice appears to be upside down.  These observations combined suggest that the affine matrix is either incorrect or is being interpreted incorrectly by the [visualization function](http://nipy.org/niwidgets/api/niwidgets.NiftiWidget.html). At the moment, there's not much we can do about this, so let's move on to consider some other features of the visualization.
...
As a general note that is particular to brain parcellations, the anatomical entities represented in a parcellation will vary depending on what method and/or "ontology" (list of anatomical entities which are considered to "exist" for the purposes of a given parcellation) is applied.  For example, the brain regions represented in the parcellation presented above are derived from the ["Destrieux" 2009 atlas](https://dx.doi.org/10.1016%2Fj.neuroimage.2010.06.010).  The parcelled regions in this atlas (and the majority of other brain atlases, for that matter) correspond to brain regions which exhibit structural, functional, and/or microstructural homogenities relative to their neighboring brain regions ([Eickhoff, Yeo, Genon, et al. 2018](https://doi.org/10.1038/s41583-018-0071-7)).

Now that we have explored three dimensional parcellations a bit, let's move on to a consideration of these NIfTI data objects are different from [tractography](https://en.wikipedia.org/wiki/Tractography) data objects.